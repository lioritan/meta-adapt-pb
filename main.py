...
def get_parser():
    ...
    parser.add_argument('--dataset', default="mini-imagenet", choices=["mini-imagenet", "omniglot", "mnist", "tiered-imagenet"],
                        help="Dataset to use.")
    parser.add_argument('--algorithm', default="bayesian-vi-two-level", choices=["train-on-test", "maml", "bayesian-vi", "meta-adaptation", "vampire", "bmaml", "bayesian-vi-two-level"],
                        help="algorithm to use.")
    parser.add_argument('--train_sample_size', default=20, type=int,
    ...
    parser.add_argument('--per_task_lr', default=1e-2, type=float,
    ...
    parser.add_argument('--n_test_epochs', default=1000, type=int,
    ...
    parser.add_argument('--test_set_mult', default=4, type=int,
                        help="relative size of evaluation vs adaptation test sets")

    # Note: boolean values don't work in argparse, so if this is set it is always true
    parser.add_argument('--meta_adaption_adaptive_kl_factor', default=1e-8, type=float,
                        help="Hyper KL factor in meta-testing vs adaptive prior")
    parser.add_argument('--meta_adaptation_hyper_kl_factor', default=1e-8, type=float,
    ...
    parser.add_argument('--vampire_kl_weight', default=1e-8, type=float,
                        help="Relative KL weight")
    parser.add_argument('--vampire_num_models', default=10, type=int,
                        help="number of models/MC averages for vampire")
    parser.add_argument('--vampire_num_models_test', default=10, type=int,
    ...
    parser.add_argument('--optimizer_lr_decay_epochs', default=10000, type=int,
                        help="Number of epochs until lr decay for step schedule")
    parser.add_argument('--optimizer_lr_schedule_type', default="step", choices=["no_change", "step"],
...
if __name__ == "__main__":
    ...
    wandb.init(project="meta-pb-inet-metaadapt4")
    ...
    algorithm.meta_learner.config["logdir"] = f"artifacts/mini-imagenet/vi/{args.seed}" #TODO: TMP
