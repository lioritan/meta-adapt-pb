...
config[
    'episode_file'] = None  # path to a csv file with row as episode name and column as list of classes that form an episode

# Log
config['logdir'] = os.path.join('/media/n10/Data', 'meta_learning', config['ml_algorithm'], config['datasource'],
...
class MLBaseClass(object):
    ...
    @abc.abstractmethod
    def adaptation(self, x: torch.Tensor, y: torch.Tensor, model: dict) -> typing.Union[
        higher.patch._MonkeyPatchBase, torch.Tensor]:
    ...
    @abc.abstractmethod
    def prediction(self, x: torch.Tensor, adapted_hyper_net: typing.Union[torch.Tensor, higher.patch._MonkeyPatchBase],
                   model: dict) -> typing.Union[torch.Tensor, typing.List[torch.Tensor]]:
    ...
    @abc.abstractmethod
    def validation_loss(self, x: torch.Tensor, y: torch.Tensor,
                    ...
                    if ((eps_count + 1) % self.config['minibatch'] == 0):
                        ...
                        if (eps_count + 1) % self.config['minibatch_print'] == 0:
                            ...
                            if val_dataloader is not None:
                                ...
                                tb_writer.add_scalar(tag="Val_NLL", scalar_value=np.mean(loss_temp),
    ...
    def evaluate(self, num_eps: int, eps_dataloader: torch.utils.data.DataLoader, model: dict) -> typing.Tuple[
