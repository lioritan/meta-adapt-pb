...
from meta_learning.vampire.CommonModels import FcNet, CNN, ResNet10, ResNet18
from meta_learning.vampire.HyperNetClasses import IdentityNet
...
class Maml(MLBaseClass):
        ...
        if resume_epoch > 0:
            ...
            saved_checkpoint = torch.load(
                ...
                map_location=lambda storage,
                                    loc: storage.cuda(self.config['device'].index) if self.config[
                                                                                          'device'].type == 'cuda' else storage
            )
    ...
    def prediction(self, x: torch.Tensor, adapted_hyper_net: higher.patch._MonkeyPatchBase,
