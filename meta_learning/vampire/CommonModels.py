...
class FcNet(torch.nn.Module):
    ...
    def construct_network(self):
        ...
        net.add_module(
            name='classifier',
            module=torch.nn.Linear(in_features=self.num_hidden_units[-1],
                                   out_features=self.dim_output) if self.dim_output is not None \
...
class CNN(torch.nn.Module):
    ...
    def __init__(self, dim_output: typing.Optional[int] = None, bn_affine: bool = False,
                 stride_flag: bool = True) -> None:
    ...
    def construct_network(self) -> torch.nn.Module:
        ...
        for i in range(1, len(self.num_channels)):
            ...
            if not self.stride_flag:
                temp.add_module(name="max-pool-{0:d}".format(i),
...
class ResNet10(torch.nn.Module):
    ...
    def modified_resnet10(self) -> torch.nn.Module:
        ...
        net = torchvision.models.resnet._resnet(arch="resnet10", block=torchvision.models.resnet.BasicBlock,
                                                layers=[1, 1, 1, 1], pretrained=False, progress=False)
        ...
        net.layer2[0].downsample[1] = torch.nn.BatchNorm2d(128, momentum=1, track_running_stats=False,
                                                           affine=self.bn_affine)
        ...
        net.layer3[0].downsample[1] = torch.nn.BatchNorm2d(256, momentum=1, track_running_stats=False,
                                                           affine=self.bn_affine)
        ...
        net.layer4[0].downsample[1] = torch.nn.BatchNorm2d(512, momentum=1, track_running_stats=False,
                                                           affine=self.bn_affine)
