...
# code from https://github.com/cnguyen10/few_shot_meta_learning/blob/master/Bmaml.py

from meta_learning.vampire.MLBaseClass import MLBaseClass
from meta_learning.vampire.HyperNetClasses import EnsembleNet
...
class Bmaml(MLBaseClass):
    ...
    def adaptation(self, x: torch.Tensor, y: torch.Tensor, model: dict) -> typing.List[higher.patch._MonkeyPatchBase]:
        ...
        for _ in range(self.config["num_inner_updates"]):
            distance_NLL = torch.empty(size=(self.config["num_models"], model["hyper_net"].num_base_params),
