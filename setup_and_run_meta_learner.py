...
from meta_learner_module_pb import MetaLearner
...
def run_meta_learner(
    ...
    if dataset == "mnist":
        task_sets = MnistDataset(data_path='~/data', train_shots=train_sample_size,
                                 ...
                                 test_ways=n_ways,
                                 shuffle_pixels=(not mnist_permute_labels),
                                 permute_labels=mnist_permute_labels,
                                 n_pixels_to_change_train=mnist_pixels_to_permute_train,
    ...
    if dataset == "mini-imagenet":
        stochastic_model = models.stochastic_models.get_model(dataset, log_var_init, input_shape=INET_SHAPE,
                                                              output_dim=n_ways)
        stochastic_ctor = lambda x: models.stochastic_models.get_model(dataset, log_var_init, input_shape=INET_SHAPE,
                                                                       output_dim=n_ways)
    elif dataset == "omniglot":
        stochastic_model = models.stochastic_models.get_model(dataset, log_var_init, input_shape=MNIST_SHAPE,
                                                              output_dim=n_ways)
        stochastic_ctor = lambda x: models.stochastic_models.get_model(dataset, log_var_init, input_shape=MNIST_SHAPE,
                                                                       output_dim=n_ways)
    else:
        stochastic_model = models.stochastic_models.get_model(dataset, log_var_init, input_shape=MNIST_SHAPE,
                                                              output_dim=n_ways)
